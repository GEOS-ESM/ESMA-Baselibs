...
FMS.config: netcdf.install netcdf-fortran.install libyaml.install
	...
	@mkdir -p ./FMS/build
	@(cd ./FMS; \
		cmake -B build -S . -DCMAKE_INSTALL_PREFIX=$(prefix)/FMS -DCMAKE_PREFIX_PATH=$(prefix) -D32BIT=ON -D64BIT=ON -DFPIC=ON -DCONSTANTS=GEOS -DNetCDF_ROOT=$(prefix) -DNetCDF_INCLUDE_DIR=$(prefix)/include/netcdf -DPORTABLE_KINDS=ON )
	@touch $@
